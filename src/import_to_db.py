...
def create_schema() -> None:
    """Create all tables from schema/schema.sql if they don't exist yet."""
...
def serial_to_iso(value, origin="1899-12-30"):
    """Convert Google/Excel serial date (number) to ISO date string.

    If *value* is already an ISO-like string it is returned as-is.
    """
    if value is None or value == "":
        return None
    # Already an ISO / human-readable string?
...
def import_data() -> None:
    """Read every data/*.json and insert rows into the SQLite DB."""
    create_schema()
    with sqlite3.connect(DB) as conn:
